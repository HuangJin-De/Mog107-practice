...
data_all= np.zeros((year_num, 365, maxZ, maxGY, maxGX)) # grid mean column condensed water (CCW)
for year in years:
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    ...
    for mon in np.arange(0,12,1):
        ...
        for day in np.arange(0,Nday,1):
            ...
            GCM_file= SPCAM_path+'CPL64.cam.h0.'+cur_date+'-00000.nc'
            with Dataset(GCM_file,'r') as D:
                T= D.variables['T'][:]
            ...
            data= Cp*T + Lv*Q + g*Geo # MSE (975 hPa)
#            data2= Cp*T + Z # DSE
#            data3= Lv * Q # LHT
            data_all[year-years[0], int(cur_day), :, : ,:]= data

with open('/data/cloud/Goldtimes5/data/GCM_SPCAM/CPL64/MSE.pk', 'wb') as fs:
