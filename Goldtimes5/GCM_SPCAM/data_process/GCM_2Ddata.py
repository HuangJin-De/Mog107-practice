...
#var_names= np.array(['FLNT','FLNS','FSNTOA','FSNS','LHFLX','SHFLX'])
var_names= np.array(['OMEGA500'])
    ...
    for mon in np.arange(0,12,1):
        ...
        for day in np.arange(0,Nday,1):
            ...
            with Dataset(CRM_file,'r') as D:
                data= D.variables[var_name][:]
                data= np.squeeze(np.mean(data,0))

            data_all[year-years[0], int(cur_day), : ,:]= data

  filesave= '/data/cloud/Goldtimes5/data/GCM_SPCAM/CPL64/'+var_name+'.pk'
