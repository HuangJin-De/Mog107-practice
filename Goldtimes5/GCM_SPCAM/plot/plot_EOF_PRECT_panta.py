...
data_path_P= '/data/cloud/Goldtimes5/data/GCM_SPCAM/CPL64/EOF_PRECT_panta.pk'
...
plt.plot(np.arange(1,5*365+1,5) ,prr-np.mean(prr),'k-',linewidth=2, label= 'Precipitation')
plt.plot(np.arange(1,5*365+1,5) ,np.sum(PC[0:3,:],axis=0),'b-',linewidth=1, label= 'Sum PC1-3')
plt.plot(np.arange(1,5*365+1,5) ,np.sum(PC[:,:],axis=0),'r-',linewidth=1, label= 'Sum PC1-100')
ax=plt.gca()
ax.set_xlim(1, 366)
ax.set_ylim(-20, 30)
plt.legend(loc='upper right', fontsize=8)
picsave= '/data/cloud/Goldtimes5/pic/GCM_SPCAM/PRECT_EOF_panta/PRECT_PC_explain.png'
plt.savefig(picsave)
plt.show()
plt.close()
...
picsave= '/data/cloud/Goldtimes5/pic/GCM_SPCAM/PRECT_EOF_panta/PRECT_EOF_tropmean.png'
plt.savefig(picsave)
plt.show()
plt.close()
...
#  plt.colorbar().set_ticks(clevel)
  ax.set_xlim(-180, 180)
  ax.set_ylim(-30, 30)
  picsave= '/data/cloud/Goldtimes5/pic/GCM_SPCAM/PRECT_EOF_panta/EOF'+str(i+1).zfill(2)+'.png'
  plt.savefig(picsave)
  plt.show()
  plt.close()
  plt.figure(figsize=(8,3))
  plt.plot(np.arange(1, 6, 5/365), np.zeros(RCEr.size),'k',linewidth=0.5)
  plt.plot(np.arange(1, 6, 5/365) , PCp ,'b-',linewidth=1)
  ax=plt.gca(); ax.set_xlim(1, 6-1/365); ax.set_ylim(-3, 3)
  picsave= '/data/cloud/Goldtimes5/pic/GCM_SPCAM/PRECT_EOF_panta/PC'+str(i+1).zfill(2)+'.png'
  plt.savefig(picsave)
  plt.show()
  plt.close()
...
picsave= '/data/cloud/Goldtimes5/pic/GCM_SPCAM/PRECT_EOF_panta/EOF_colorbar.png'
plt.savefig(picsave)
