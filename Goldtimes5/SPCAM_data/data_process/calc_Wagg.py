    ...
    for mon in np.arange(0,12):
        ...
        for day in np.arange(0,Nday):
            ...
            with Dataset(CRM_file,'r') as D:
                data= D.variables['CRM_W_LON_60e_to_180e_LAT_15s_to_30n']
                data= np.squeeze(np.mean(data,0))
                for gy in np.arange(0,maxGY):
                    for gx in np.arange(0,maxGX):
                        temp= np.squeeze(data[16,:,gy,gx])
                        ind= temp.argsort()[-6:][::-1]
                        dist= np.zeros(15)
                        ni=0
...
#                        data_all[year-years[0], int(cur_day), gy, gx]= np.mean(hm)
                        data_all2[year-years[0], int(cur_day), gy, gx]= np.var(temp,ddof=1)
...
with open('/data/cloud/Goldtimes5/data/SPCAM/CPL64/Wvar.pk', 'wb') as fs:
     pickle.dump(data_all2, fs, protocol = 4)

with open('/data/cloud/Goldtimes5/data/SPCAM/CPL64/Wagg.pk', 'wb') as fs:
