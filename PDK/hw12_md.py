...
m = Basemap(projection='cyl',llcrnrlat=-30,urcrnrlat=30,llcrnrlon=0,urcrnrlon=360)
...
ATM_rad = top_flux - sfc_flux

nc = Dataset('/data/dadm1/reanalysis/ECMWF/ITM/daily/SH/daily_interim_SH_2000.nc')


sfc_sh = nc.variables['sshf'][:,80:161,:]/86400
LAT2 = nc.variables['latitude'][:]

nc = Dataset('/data/dadm1/obs/TRMM_PR/2A25/2A25_2000.nc')
print(nc.variables)

latent_h = np.nanmean(nc.variables['Precipitation'][:,:,:,:],axis=0)/1000/3600*1000*2.25e6
LAT3 = nc.variables['latitude'][:]

AA = np.mean(np.mean(ATM_rad,axis=1),axis=1) + np.mean(np.mean(sfc_sh,axis=1),axis=1) + np.mean(np.mean(latent_h,axis=1),axis=1)
