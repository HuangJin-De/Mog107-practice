...
path = '/data/cloud/ucantseeme/hw06/'
filename = 'CRH_OLR_freq_big.nc'
...
m2 = ax.contourf(CRH, OLR, w_2pdf, cmap='seismic', levels=levels, vmin=vmin, vmax=vmax, extend='both')
ax.contour(CRH, OLR, w_2pdf, levels=0, colors='black')
levels = np.zeros(10)
vmin = -7; vmax = -1; n = 10
...
ax.set_title('CRH, OLR joint PDF (%) & midlevel omega')
#fig.colorbar(m, cax=ax_cb)
fig.colorbar(m2, cax=ax_cb, label='Pa/s')
...
plt.savefig('CRH_OLR_freq_omega.png', dpi=300)

# In[]
region = ['A', 'B', 'C']
MAP = [map_a, map_b, map_c]
for i in range(len(region)):
    ...
    cx,cy = m(LON,LAT)
    CS = m.pcolormesh(cx,cy,MAP[i]*100,cmap = 'YlGnBu', shading='flat')
    m.colorbar(CS)
