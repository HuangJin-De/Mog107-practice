...
#  z    = ds['Z3'][:, :, latindex, lonindex]
  ...
  for y in [4]:#range(np.size(latrange)-1):
    for x in [5]:#range(np.size(lonrange)-1):
      ...
      wt = w.reshape(1, 1, indy.size, 1)
      w_ext = np.tile(wt, [24, 20, 1, indx.size])

      #data = Tnew[:, :, indy[0]:indy[-1]+1, indx[0]:indx[-1]+1]
      #masked = ~np.isnan(data)
      #Tmean = np.zeros([24, 20])
      #for t in range(24):
      #  for k in range(20):
      #    if np.all(np.isnan(data[t, k, :, :])):
      #      Tmean[t, k] = np.nan
      #    else:
      #      Tmean[t, k] = np.average(data[t, k, :, :][masked[t, k, :, :]], weights=w_ext[masked[t, k, :, :]])
      ...
      data = qnew[:, :, indy[0]:indy[-1]+1, indx[0]:indx[-1]+1]
      masked = ~np.isnan(data)
      qmean = np.nansum(data*w_ext*masked, axis = (2,3))/np.sum(w_ext*masked, axis = (2,3))
      print(qmean[:,0])
      #qmean = np.zeros([24, 20])
      #for t in range(24):
      #  for k in [0]:#range(20):
      #    if np.all(np.isnan(data[t, k, :, :])):
      #      qmean[t, k] = np.nan
      #    else:
      #      qmean[t, k] = np.average(data[t, k, :, :][masked[t, k, :, :]], weights=w_ext[masked[t, k, :, :]])
...
if rank == 0:
  data_qv.astype(np.float32).tofile('qv_spcam_10x10_extend2.dat')
  data_qvs.astype(np.float32).tofile('qvs_spcam_10x10_extend2.dat')
